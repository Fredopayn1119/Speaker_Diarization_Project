...
# Suppress PyTorch warnings about custom classes that are causing issues
import torch
torch._C._jit_set_profiling_executor(False)
torch._C._jit_set_profiling_mode(False)

# Suppress other related warnings
...
# Initialize session state for storing transcript data
...
def update_transcript_with_names():
    """Update the transcript with custom speaker names"""
...
if audio and st.button("Generate Transcript"):
    with st.spinner("Running speaker diarization..."):
        ...
        try:
            ...
            # Store speaker segments in session state
            ...
            # Initialize speaker_names with empty values
            st.session_state.speaker_names = {speaker_id: "" for speaker_id in st.session_state.speaker_segments.keys()}
            
            # Combine all segments from all speakers into a single list
            all_segments = []
            ...
            # Generate the transcript in chronological order
            return_value = []
            for segment in all_segments:
